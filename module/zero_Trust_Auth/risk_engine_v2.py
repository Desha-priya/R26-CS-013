...
MODELS_DIR = "models"
OUTPUT_FILE = "output/risk_engine_result.txt"
...
class RiskEngine:
    ...
    SECURITY_POLICIES = {
        "high_risk_keywords": ["transfer", "payment", "bank", "withdraw", "deposit", 
                             "otp", "password", "admin", "credit", "debit", "invoice", "pay", "transaction"],
        "sensitive_contexts": ["banking", "finance", "payment", "admin", "transaction", "netbanking"]
    }
    ...
    def __init__(self):
        ...
        self.scaler        = joblib.load(os.path.join(MODELS_DIR, "scaler_v2.pkl"))
        self.iso_forest    = joblib.load(os.path.join(MODELS_DIR, "isolation_forest_v2.pkl"))
        self.oc_svm        = joblib.load(os.path.join(MODELS_DIR, "oneclass_svm_v2.pkl"))
        self.user_profiles = joblib.load(os.path.join(MODELS_DIR, "user_profiles_v2.pkl"))

        weights_path = os.path.join(MODELS_DIR, "feature_weights_v2.pkl")
    ...
    def is_sensitive_action(self, context: str = "normal_browsing") -> bool:
        if not context or context == "normal_browsing":
            return False
        ctx = context.lower()
        for kw in self.SECURITY_POLICIES["high_risk_keywords"]:
            if kw in ctx:
                return True
        for sc in self.SECURITY_POLICIES["sensitive_contexts"]:
            if sc in ctx:
                return True
        return False    
    ...
    def score_session(self, live_features: list, user_id: int,
        ...
        # NEW: Apply Security Policy Boost
        if self.is_sensitive_action(context):
            instant_score = min(1.0, instant_score * 1.65)   # Strong boost for banking etc.
        ...
        return {
            ...
            'window_size':        0,
            'mode':               'replay',
            'is_sensitive_action': self.is_sensitive_action(context),   # Add this line
            'policy_boost': 1.65 if self.is_sensitive_action(context) else 1.0,
        }
...
#'''
    ...
    log_to_file ("--- TEST 1: Normal user, no alerts ---")
    ...
    r = engine.score_session(user1, 1, 'normal_browsing', use_window=False)
    log_to_file(f"Risk: {r['risk_percent']}% | Boost: {r['alert_boost']} | Threshold: {r['adaptive_threshold']} | Decision: {r['decision']}")

    print("\n--- TEST 3: Anomaly user + all alerts + sensitive context ---")
    ...
    log_to_file(f"User {anomaly_uid} | Risk: {r['risk_percent']}% | Decision: {r['decision']}")
    ...
    log_to_file(f"Same typing: Risk: {r['risk_percent']}% | Decision: {r['decision']}")
    ...
    log_to_file(f"Different typing: Risk: {r['risk_percent']}% | Decision: {r['decision']}") 
